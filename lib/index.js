...
module.exports = new class {
    ...
    get(path, query) {
        ...
        if (typeof order === 'string' && order.indexOf('/') >= 0) {
            order = order.split('/')
        }
        if (order === '$key') {
    ...
    sortedKeys(node, order, reverse) {
        ...
        if (order === '$key') {
            keys = keys.map(value => this.intForValue(value))
        }
        return keys.sort((a, b) => this.compareValues(this.valueFor(node, order, a) || '', this.valueFor(node, order, b) || '', reverse))
    }
